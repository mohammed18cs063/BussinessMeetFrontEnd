...
export class FoodMenuService {
  ...
  Addingtheme(addfood : FoodMenuModel ) :Observable<Object>
  {
    console.log(addfood);
    
    return this.httpClient.post<any>(`http://localhost:8080/admin/FoodMenu/addFoodMenu`,addfood);
  }

  getUsersList(): Observable<FoodMenuModel[]>{
    return this.httpClient.get<FoodMenuModel[]>(`http://localhost:8080/admin/FoodMenu/getFoodMenus`);
  }

  deleteUser(id:number): Observable<Object>{
    return this.httpClient.delete(`http://localhost:8080/admin/FoodMenu/deleteFoodMenu/${id}`);
  }
  updateUser(id: number,user:FoodMenuModel): Observable<Object>{
    return this.httpClient.put(`http://localhost:8080/admin/FoodMenu/editFoodMenu/${id}`,user);
  }
  getUserById(id:number):Observable<FoodMenuModel>{
    return this.httpClient.get<FoodMenuModel>(`http://localhost:8080/admin/FoodMenu/getFoodMenu/${id}`);
  }
