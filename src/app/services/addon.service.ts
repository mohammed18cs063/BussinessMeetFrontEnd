...
export class AddonService {
  ...
  AddingAddOn(addaddon: AddonModel): Observable<Object> {
    ...
    return this.httpClient.post<any>(
      `http://localhost:8080/admin/addAddon`,
      addaddon
    );
  }

  getAddons(): Observable<AddonModel[]> {
    return this.httpClient.get<AddonModel[]>(
      `http://localhost:8080/admin/addons`
    );
  }

  deleteAddon(id: number): Observable<Object> {
    return this.httpClient.delete(
      `http://localhost:8080/admin/deleteAddon/${id}`
    );
  }
  updateAddon(id: number, user: AddonModel): Observable<Object> {
    return this.httpClient.put(
      `http://localhost:8080/admin/editAddon/${id}`,
      user
    );
  }
  getAddonById(id: number): Observable<AddonModel> {
    return this.httpClient.get<AddonModel>(
      `http://localhost:8080/admin/addon/${id}`
    );
  }
}
